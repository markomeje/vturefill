...
class Levels extends Model {
	...
	public static function addLevel() {
		$posted = ["level" => self::post("level"), "status" => self::post("status"), "minimum" => self::post("minimum"), "maximum" => self::post("maximum")];
		if (empty($posted["level"]) || !is_numeric($posted["level"])) {
		...
		}elseif (empty($posted["minimum"]) || !is_numeric($posted["minimum"])) {
			return ["status" => "invalid-minimum"];
		}elseif (empty($posted["maximum"]) || !is_numeric($posted["maximum"])) {
